...
my ($input_dir,$pirate_dir,$steps,$pan_options);
...
$pirate_dir = "$input_dir/PIRATE" unless defined $pirate_dir;
unless( -d $pirate_dir ) {
    mkdir "$pirate_dir" || die " - ERROR: could not make working directory in $pirate_dir\n";
}
$pirate_dir = abs_path($pirate_dir);

# check for mandatory input arguments

pod2usage( { -message => q{input directory is a required arguement}, 
	     -exitval => 1, 
	     -verbose => 1 } ) unless defined $input_dir;

# check for presence of input/output directories.
pod2usage( {-message => " - ERROR: input directory: $input_dir is not a directory", 
	    -exitval => 1, 
	    -verbose => 1 } ) unless -d $input_dir; 

# Check for > 1 gff files in input directory.
opendir(DIR, $input_dir) || die ("cannot open dir $input_dir: $!");
my @files;
for my $file ( readdir(DIR) ) {
    push @files, $file if ($file =~ /\.gff/); # could check if ends with .gff or .gff3 only?
}
close DIR;
my $no_files = scalar(@files);

pod2usage( {-message => " - ERROR: $no_files files with .gff extension in $input_dir", 
	    -exitval => 1, 
	    -verbose => 1 } ) unless $no_files > 1; # need at least 2 files 

# sanity check aa% id thresholds.
my @thresholds=();
unless ( defined $steps ) {
    @thresholds= qw(50 60 70 80 90 95 98);
    $steps=join(",", @thresholds);
} else{
    @thresholds= sort( {$a<=>$b} split( /\s*,\s*/, $steps) );
    $steps=join(",", @thresholds);
    for ( @thresholds ) {
	pod2usage( {-message => " - ERROR: $_ is not an integer in threshold list", 
		    -exitval => 1, -verbose => 1 } ) if $_ !~ /\d+\z/;
	pod2usage( {-message => " - ERROR: $_ is not between 0-100%", 
		    -exitval => 1, -verbose => 1 } ) if $_ > 100;
	pod2usage( {-message => " - ERROR: $_ is not between 0-100%", 
		    -exitval => 1, -verbose => 1 } ) if $_ < 0;
    }
}
...
my $genome2loci = File::Spec->catfile($pirate_dir,"genome2loci.tab");
my $it_dir      = File::Spec->catdir($pirate_dir,"pangenome_iterations");
my $gff_dir     = File::Spec->catdir($pirate_dir,"modified_gffs");
...
if ( $pan_off == 0 ) {
    ...
    # make directory
    unless( -d $gff_dir ) { 
	mkdir $gff_dir || die(" - ERROR: could not make PIRATE gff directory in $pirate_dir\n");
    }

    # clear existing gffs
    unlink glob "$gff_dir/*.gff";
    open( my $logfile => ">$pirate_dir/gff_parser_log.txt") || die ("cannot open parser log $pirate_dir/gff_parser_log.txt: $!");
    open(my $fh => "| parallel -j $threads \"perl $script_path/parse_GFF.pl {} >& $pirate_dir/gff_parser_log.txt\"") || die $!;
    opendir(INDIR,$input_dir) || die "cannot open $input_dir: $!";
    for my $file ( readdir(INDIR) ) {
	if ($file =~ /\.gff/) {
	    print $fh "$input_dir/$file\n";
	}
    }
    close(INDIR);
    ...
    # make genome list
    open(G_LIST, ">".File::Spec->catfile($pirate_dir,"genome_list.txt")) || die "Cannot open $pirate_dir/genome_list.txt: $!";
    for my $g ( @files ) { 
	if ( $g =~ /(.+).gff/) { 
	    print G_LIST "$1\n"; 
	}
    ...
    $time_start = time();
    my $coords_dir = File::Spec->catfile($pirate_dir,"co-ords");
    unless( -d "$coords_dir" ) {
	mkdir "$coords_dir" || die "\n - ERROR: could not make PIRATE co-ords directory in $pirate_dir\n";
    }

# this expects contiguous features?
